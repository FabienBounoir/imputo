<script lang="ts">
	...
	type Entity = 'ALL' | 'TICKET' | 'ABSENCE' | 'MEMBER' | 'WORKSPACE';
	...
	function entityLabel(e: Entry) {
		if (e.entityType === 'TICKET') return 'Ticket';
		if (e.entityType === 'WORKSPACE') return 'Configuration';
		if (e.entityType === 'MEMBER') return e.subjectName ?? 'Membre';
		return e.subjectName ? `Absence · ${e.subjectName}` : 'Absence';
	}
	...
	// liste devient visible, plutôt que tout charger d'un coup (fenêtre de rétention qui peut être dense).
	$effect(() => {
		if (!sentinel) return;
...
<div class="topbar">
	<h1>Historique<small>Modifications tracées : tickets, absences, membres, configuration — {data.windowDays} derniers jours</small></h1>
</div>

<div class="content">
	<div class="hfilters">
		<input class="hsearch" placeholder="Rechercher (ticket, personne, activité, valeur…)" bind:value={query} oninput={onSearchInput} />
		<div class="htabs">
			...
			<button type="button" class:active={entityFilter === 'MEMBER'} onclick={() => setEntityFilter('MEMBER')}>Membres</button>
	...
	<section class="card block">
		{#if entries.length === 0}
			<p class="hint" style="margin:0;">
				{#if loading}Chargement…{:else if query.trim() || entityFilter !== 'ALL'}Aucun résultat pour ces filtres.{:else}Aucune modification tracée sur les {data.windowDays} derniers jours.{/if}
			</p>
		{:else}
			<div class="hlist">
				{#each entries as e (e.id)}
					<div class="hrow">
						...
						{:else}
							<span class="hfield">{fieldLabel(e.entityType, e.field)}{#if e.activityLabel} · {e.activityLabel}{/if}</span>
							{#if e.oldValue === null && e.newValue === null}
								<span class="hchange">Modifié</span>
							{:else}
								<span class="hchange">{formatChangeValue(e.entityType, e.field, e.oldValue)} → {formatChangeValue(e.entityType, e.field, e.newValue)}</span>
							{/if}
						{/if}
			...
			{:else if !loading}
				<p class="hsentinel hint">Fin des {data.windowDays} derniers jours.</p>
			{/if}
		{/if}
	</section>
</div>
...
<style>
	...
	.hchange {
		color: var(--text);
		/* un JQL ou un titre long ne doit pas élargir la ligne */
