...
describe('runSnapshot', () => {
	it('fige estimation/RAE/consommé du jour pour chaque ticket', async () => {
		...
		await updateTicketField(a.workspaceId, t.id, 'estimationReal', '5');
		await updateTicketField(a.workspaceId, t.id, 'raeReal', '3');
		...
		await updateTicketField(a.workspaceId, t.id, 'raeReal', '1');
		await runSnapshot(dateISO, a.workspaceId);
