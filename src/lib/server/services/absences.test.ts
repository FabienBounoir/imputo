...
describe('buildAbsenceGrid', () => {
	it('place une absence d\'un jour sur le bon jour', () => {
		...
		expect(grid.u1['2026-06-01']).toEqual({ type: 'CONGE_VALIDE', period: 'FULL' });
	...
	it('garde le type et la demi-journée sur chaque cellule', () => {
		...
		expect(grid.u1['2026-06-01']).toEqual({ type: 'FORMATION', period: 'PM' });
	});

	it('indexe séparément un membre externe (subjectId distinct)', () => {
		const grid = buildAbsenceGrid(
			[abs({ subjectId: 'ext1', external: true, displayName: 'Client X' })],
			['2026-06-01']
		);
		expect(grid.ext1['2026-06-01']).toEqual({ type: 'CONGE_VALIDE', period: 'FULL' });
	});
});
