...
import { db, ticket, timeEntry, jiraSyncRun, sprint, ticketSprintMember } from '$lib/server/db';
import {
	createTicket,
	listTickets,
	updateTicketField,
...
describe('deleteUntouchedSyncedTickets', () => {
	...
	it('supprime un ticket rattaché à un sprint/version par le sync — pas un signe de trace humaine (régression)', async () => {
		const { workspaceId } = await makeWorkspace();
		const runId = await makeSyncRun(workspaceId);
		...
		const [t] = await db
			.insert(ticket)
			.values({ workspaceId, key: 'K-SPRINTVER', title: 'x', createdBySyncRunId: runId })
			.returning({ id: ticket.id });
		await db.insert(ticketSprintMember).values([
			{ ticketId: t.id, sprintId: ver.id },
			{ ticketId: t.id, sprintId: spr.id }
		]);
