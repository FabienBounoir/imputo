<script lang="ts">
	...
	type SprintDashboard = {
		...
		kpis: {
			...
			/** Admin only — même valeur que ecartVsBudgetTotal, libellé historique "TNF". */
		...
		<div class="kpis">
			...
			{#if dashboard.kpis.tnfBudgetTotal !== null}
				<div class="card kpi" class:warn={dashboard.kpis.tnfBudgetTotal > 0}>
					<div class="k">TNF</div>
					<div class="v tabnum">{dashboard.kpis.tnfBudgetTotal > 0 ? '+' : ''}{dashboard.kpis.tnfBudgetTotal}<small>j</small></div>
				</div>
			{/if}
		</div>
